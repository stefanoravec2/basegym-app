...
export default function Calendar() {
  ...
  async function loadData() {
    ...
    const { data: tr } = await supabase
      .from('trainings')
      .select('*, reservations(id, client_id, status, client_profiles(full_name))')
  ...
  async function reserve(training) {
    if (!credits || credits.amount <= 0) {
      setActionMsg('Nemáš dostatok kreditov. Kontaktuj trénera.')
    ...
    if (!error) {
      ...
      setActionMsg('Rezervácia úspešná!')
    } else {
      setActionMsg('Chyba. Skús znova.')
    }
  ...
  async function cancel(training) {
    if (!canCancel(training)) {
      setActionMsg('Odhlásenie nie je možné menej ako 30 minút pred tréningom.')
      setTimeout(() => setActionMsg(''), 4000)
      return
    }
    ...
    if (credits) {
      ...
      await supabase.from('credit_logs').insert({
        ...
        reason: `Zrušenie rezervácie: ${training.title}`,
        training_id: training.id
      })
    }
    setActionMsg('Rezervácia zrušená. Kredity vrátené.')
    setTimeout(() => setActionMsg(''), 3000)
    loadData()
  }
  ...
  return (
    <div>
      {/* Credits bar */}
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '14px 20px', marginBottom: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px' }}>
        <div>
        ...
        {credits ? (
          <div style={{ textAlign: 'right' }}>
            ...
            {new Date(credits.expires_at) < new Date(Date.now() + 7 * 86400000) && (
              <span className="badge badge-amber" style={{ marginTop: '4px' }}>Expiruje čoskoro</span>
            )}
          </div>
        ) : (
      ...
      {actionMsg && <div className="info-box info-green" style={{ marginBottom: '16px' }}>{actionMsg}</div>}
              ...
              <div key={day.toDateString()} style={{ opacity: isPast ? 0.5 : 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                  <div style={{
                    ...
                    background: isToday ? '#1A1A1A' : 'var(--bg)',
                  ...
                  {dayTrainings.map(t => {
                    ...
                    return (
                      <div key={t.id} style={{
                        ...
                        border: `1.5px solid ${reserved ? 'var(--green)' : 'var(--border)'}`,
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <button onClick={() => setExpanded(isExpanded ? null : t.id)} style={{
                              ...
                              {activeCount}/{t.capacity} miest
                            </button>

                            {!past && (
                              reserved ? (
                                <button onClick={() => cancel(t)} className="btn btn-red" style={{ fontSize: '12px', padding: '6px 14px' }}>
                                  Odhlásiť
                                </button>
                              ) : (
                                <button onClick={() => reserve(t)} className="btn btn-green" style={{ fontSize: '12px', padding: '6px 14px' }}
                                  disabled={full || !canReserve(t)}>
                                  {full ? 'Plné' : 'Prihlásiť'}
                                </button>
                              )
                            )}
                        ...
                        {isExpanded && attendees.length > 0 && (
                          <div style={{ marginTop: '8px', borderTop: '1px solid var(--border)', paddingTop: '8px' }}>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                              {attendees.map((r, i) => (
                                <span key={i} style={{ fontSize: '12px', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--radius-full)', padding: '3px 10px', color: 'var(--text)' }}>
